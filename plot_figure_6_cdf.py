...
prevalence_axis.step(xs,1-ns*1.0/ns[0],'k-',label='Random')

xs, ns = stats_utils.calculate_unnormalized_survival_from_vector(between_host_gene_prevalences)
prevalence_axis.step(xs,1-ns*1.0/ns[0],'r-',label='Between-host')
...
if len(low_divergence_between_host_gene_prevalences) > 0:
    ...
    prevalence_axis.step(xs,1-ns*1.0/ns[0],'r-',label=('d<%g' % modification_divergence_threshold), alpha=0.5)

xs, ns = stats_utils.calculate_unnormalized_survival_from_vector(within_host_gene_prevalences)
prevalence_axis.step(xs,1-ns*1.0/ns[0],'g-',label='Within-host')
